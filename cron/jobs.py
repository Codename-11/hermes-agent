...
# Cron definitions live in one root registry. Owner metadata isolates claims and execution.
...
def get_active_cron_profile() -> str:
    ...
    try:
        active, root = get_hermes_home().resolve(), _cron_registry_root()
        if active == root:
            return _DEFAULT_OWNER_PROFILE
...
def resolve_profile_home(profile: Optional[Any]) -> Optional[Path]:
    owner, root = _normalize_owner_profile(profile), _cron_registry_root()
...
def _filter_jobs_for_active_profile(
    ...
    return [job for job in jobs if _job_visible_to_active_profile(job, active_profile=active)]
...
def _ensure_job_ownership(job: Dict[str, Any], default_owner: str) -> bool:
    ...
    changed = False
    for key, value in (("owner_profile", owner), ("profile", owner), ("scope", scope)):
...
def _current_cron_store() -> _CronStorePaths:
    """Return paths for the shared root registry, honoring explicit test overrides."""
    ...
    if root == HERMES_DIR:
        return live_constants
    return _CronStorePaths.for_dir(root / "cron")


def _shared_store_home(home: Union[str, Path]) -> Path:
    candidate, root = Path(home).expanduser().resolve(), _cron_registry_root()
    try:
...
def use_cron_store(home: Union[str, Path]):
    ...
    token = _cron_store_override.set(
        _CronStorePaths.for_dir(_shared_store_home(home) / "cron"))
    try:
        yield
    finally:
...
def _mark_legacy_store_migrated(path: Path, owner: str) -> None:
    try:
        ...
        if path.exists() and not backup.exists():
            ...
            _secure_file(backup)
        payload = {
            "jobs": [], "updated_at": _hermes_now().isoformat(),
            "migrated_to_shared_store": True, "migrated_profile": owner,
...
def _merge_legacy_profile_stores(
    ...
    seen = {str(job.get("id")) for job in jobs if isinstance(job, dict) and job.get("id")}
    changed, migrated = False, []
    for owner, path in _legacy_profile_job_files():
        legacy_jobs, payload = _read_legacy_jobs(path)
        if payload.get("migrated_to_shared_store") or not legacy_jobs:
            continue
...
def _with_job(
    ...
    with _jobs_lock():
        ...
        for i, job in enumerate(jobs):
            if job.get("id") == job_id and _job_visible_to_active_profile(job):
...
def create_job(
    ...
    owner_profile = (
        _normalize_owner_profile(profile) if profile is not None else get_active_cron_profile())
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    jobs = [
        _apply_skill_fields(job) for job in copy.deepcopy(raw_jobs)
