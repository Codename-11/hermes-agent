...
`provenance` is audit metadata only. A source change is part of the generated artifact only when its immutable SHA appears in the active carry's `replay.commits` and is reachable from the declared `source_ref`. Review must inspect the exact candidate for newly added worker/validator/promotion controls; testing an external source worktree is not proof that those controls were replayed.

Manual promotion from a detached old-deploy checkout must stage the immutable run's manifest snapshot byte-for-byte before evidence validation. The live-manifest SHA is compared to the report-bound manifest SHA; Windows text-mode writes can normalize line endings and fail this gate. This staging is allowed only in the disposable promotion checkout, never in a managed/live checkout.
