...
2. **Git update** — pulls the latest code from the selected branch, or fast-forwards the current deploy branch after upstream has been merged into that branch.
3. **Post-pull syntax validation + auto-rollback** — after the pull, Hermes compiles the critical files every `hermes` invocation imports at startup. If any fails to parse (e.g. an orphan merge-conflict marker, an accidentally truncated file), Hermes runs `git reset --hard <pre-pull-sha>` to roll the install back so your shell stays bootable. Re-run `hermes update` once the upstream fix lands.
