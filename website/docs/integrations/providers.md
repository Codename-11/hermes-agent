...
:::tip Claude subscription billing
When you authenticate via `hermes model` → Anthropic OAuth (or via `hermes auth add anthropic --type oauth`), Hermes routes through the Claude Code OAuth credential lane for your Anthropic account. Claude Code usage such as `claude -p` is covered by the standard Claude subscription allowance rather than requiring separate "extra usage" credits; Hermes mirrors that lane for native Anthropic OAuth requests.

If you prefer API billing, or if your subscription/account is not eligible for Claude Code OAuth usage, use an `ANTHROPIC_API_KEY` instead — requests are billed pay-per-token against that key's organization (standard API pricing, independent of any Claude subscription).
:::
