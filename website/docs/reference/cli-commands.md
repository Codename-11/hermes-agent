...
- **Fork deploy branches.** When a forked install runs from a configured generated deploy branch such as `axiom` or `tgi`, `hermes update` treats `origin/<deploy-branch>` as the published deploy artifact. If upstream work is pending without a matching verified candidate, it starts detached carry-stack reconciliation in an isolated worktree, publishes only `origin/<deploy-branch>-next`, and returns with the current deployment unchanged. Run `hermes update` again after the candidate reports ready to archive the old deploy ref, lease-promote the exact verified candidate, realign the stashed checkout, and continue the normal install/restart path. The live host never performs an autonomous whole-branch merge.
