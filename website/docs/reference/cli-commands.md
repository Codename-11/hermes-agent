...
- **Patched deploy branches.** If you maintain a long-lived deploy branch with local runtime patches, keep `main` aligned with upstream and commit those patches on the deploy branch. A deploy-aware fork can use bare `hermes update` to resume a retained handoff or reconcile upstream in an isolated worktree; it must leave the live checkout unchanged when reconciliation fails. Prefer upstream code when it preserves the documented local outcome, retain focused behavior tests, and do not force the generic `--branch` path.
