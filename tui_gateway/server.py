...
_IMAGE_EXTENSIONS_FALLBACK = frozenset({
    ".png", ".jpg", ".jpeg", ".gif", ".webp",
    ".bmp", ".tiff", ".tif", ".svg", ".ico",
})


def _split_path_input_fallback(raw: str) -> tuple[str, str]:
...
def _resolve_attachment_path_fallback(raw_path: str) -> Path | None:
...
def _detect_file_drop_fallback(user_input: str) -> dict | None:
    ...
    direct_path = _resolve_attachment_path_fallback(stripped)
    if direct_path is not None:
        return {
            "path": direct_path,
            "is_image": direct_path.suffix.lower() in _IMAGE_EXTENSIONS_FALLBACK,
            "remainder": "",
        }

    first_token, remainder = _split_path_input_fallback(stripped)
    drop_path = _resolve_attachment_path_fallback(first_token)
    if drop_path is None and " " in stripped and stripped[0] not in {"'", '"'}:
        for pos in reversed([idx for idx, ch in enumerate(stripped) if ch == " "]):
            ...
            resolved = _resolve_attachment_path_fallback(candidate)
    ...
    return {
        "path": drop_path,
        "is_image": drop_path.suffix.lower() in _IMAGE_EXTENSIONS_FALLBACK,
        "remainder": remainder,
    }


def _attachment_helpers():
    try:
        from cli import (
            _IMAGE_EXTENSIONS,
            _detect_file_drop,
        ...
        return (
            _IMAGE_EXTENSIONS_FALLBACK,
            _detect_file_drop_fallback,
            _resolve_attachment_path_fallback,
            _split_path_input_fallback,
        )


def _detect_file_drop_helper():
    try:
        from cli import _detect_file_drop

        return _detect_file_drop
    except Exception:
        return _detect_file_drop_fallback
...
def _legacy_image_format_ext(fmt: str) -> str | None:
...
def _(rid, params: dict) -> dict:
    ...
    Supports upstream's ``image.attach_bytes`` RPC shape and the legacy Axiom /
    Hermes-Relay ``image.attach.bytes`` shape:

    - content_base64 / data + optional filename/ext
    - bytes_base64 + required format + optional filename_hint
    """
    session, err = _sess(params, rid)
    if err:
        return err

    legacy_b64 = params.get("bytes_base64")
    raw_b64 = str(params.get("content_base64") or params.get("data") or legacy_b64 or "").strip()
    if not raw_b64:
    ...
    filename = str(params.get("filename", "") or params.get("filename_hint", "") or "")
    ext_hint = str(params.get("ext", "") or "").strip().lower()
    fmt_raw = params.get("format")
    if legacy_b64 is not None:
        if not fmt_raw or not isinstance(fmt_raw, str):
            return _err(rid, 4015, "format required")
        legacy_ext = _legacy_image_format_ext(fmt_raw)
        if legacy_ext is None:
            return _err(rid, 4016, f"unsupported format: {fmt_raw!r}")
        ext_hint = legacy_ext
    ...
    if legacy_b64 is not None:
        ...
        normalized_expected = ".jpg" if ext_hint == ".jpeg" else ext_hint
        normalized_sniffed = ".jpg" if sniffed == ".jpeg" else sniffed
        if normalized_expected and normalized_sniffed != normalized_expected:
            return _err(rid, 4019, f"format mismatch — body does not look like {fmt_raw}")
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        _detect_file_drop = _detect_file_drop_helper()
