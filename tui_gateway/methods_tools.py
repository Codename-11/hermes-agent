...
def _plugin_command_entry(name: str):
    try:
        return _tools_mod("hermes_cli.plugins").get_plugin_command_entry(name)
    except Exception:
        return None


def _run_plugin_command(entry, arg: str, session: dict) -> str:
    handler = entry["handler"]
    try:
        raw = handler(arg, session_id=session.get("session_key", "") if session else "")
    except TypeError:
        raw = handler(arg)
    result = _tools_mod("hermes_cli.plugins").resolve_plugin_command_result(raw)
    if entry.get("returns_card") or isinstance(result, dict):
        cards = _tools_mod("gateway.cards")
        if cards.is_card(result):
            result = cards.render_card_as_text(result)
...
def _dispatch_plugin(rid, params, session, name, arg):
    if entry := _plugin_command_entry(name):
        with contextlib.suppress(Exception):
            return _ok(rid, {"type": "plugin", "output": _run_plugin_command(entry, arg, session)})
    return None
...
def _(rid, params: dict) -> dict:
    ...
    if plugin_entry := _plugin_command_entry(base) if base else None:
        if plugin_entry.get("gateway_only"):
            return _err(rid, 4018, f"gateway-only plugin command: use command.dispatch for /{base}")
        try:
            return _ok(rid, {"output": _run_plugin_command(plugin_entry, arg, session) or "(no output)"})
        except Exception as e:
