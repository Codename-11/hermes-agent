...
test('profile URL route preserves its remote profile alias while matching the gateway identity', () => {
  const block = {
    mode: 'remote',
  ...
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'local', profiles: { atlas: block } },
    profile: 'atlas',
    registry: registry('local', [{ id: 'atlas', kind: 'remote', label: 'Atlas', ...block }])
  })

  assert.equal(route?.kind, 'remote')
  assert.equal(route?.remoteProfile, 'default')
  assert.equal(route?.connectionId, 'atlas')
})
