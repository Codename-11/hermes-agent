 ...
 * URL overrides may pass `remoteProfile` explicitly; managed SSH routes carry
 * the same backend identity in `opts.backendProfile`.
 */
function pathWithGlobalRemoteProfile(
  ...
  const scopedProfile = connectionScopeKey(profile)
  const backendProfile = connectionScopeKey(remoteProfile) || connectionScopeKey(opts.backendProfile)
  const translated = translateSelfProfileQuery(path, profile, backendProfile)
  ...
  const requestProfile = backendProfile || (route.scopePath ? scopedProfile : null)

  if (!requestProfile) {
    return path
  }

  return pathWithProfileScope(path, requestProfile)
}
