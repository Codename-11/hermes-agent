...
/**
 * Resolve the Axiom staged-apply hand-off.
 *
 * The staged Update Control contract still needs the legacy flat script: it
 * validates the pinned manifest, atomically adopts the prepared package, and
 * restores the previous package if the deploy-aware CLI update fails. The
 * split upstream windows.ps1 orchestrator intentionally handles ordinary
 * updates only and does not accept -StageManifest.
 */
export function resolveStagedApplyScriptHandoff(
  updateRoot: string,
  deps: ResolveUpdateScriptHandoffDeps = {}
): UpdateScriptHandoff | null {
  const isWindows = deps.isWindows ?? process.platform === 'win32'
