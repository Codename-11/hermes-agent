...
async function buildRemoteConnection(rawUrl, authMode, token, source, remoteProfile = null) {
...
async function requestJsonForProfile(profile: string | null, path: string, method: Method, body?: string) {
  const conn = await ensureBackend(profile)
