...
// TGI's remote HTML preview bridge remains separate from upstream's media
// streaming protocol and must be registered before app readiness.
...
function remotePreviewRoute(remotePath, remoteProfile, includeToken = null) {
...
function safeDownloadFilename(rawName) {
  ...
  const cleaned = String(base || 'artifact')
    // Windows forbids ASCII control characters in filenames.
...
async function openRemoteFile(payload: any = {}) {
...
function fetchJson(url, token, options: any = {}): Promise<any> {
  return new Promise((resolve, reject) => {
...
function fetchBinary(url, token, options: any = {}): Promise<any> {
  return new Promise((resolve, reject) => {
    const parsed = new URL(url)
    const client = parsed.protocol === 'https:' ? https : http
    const timeoutMs = resolveTimeoutMs(options.timeoutMs, DEFAULT_FETCH_TIMEOUT_MS)
...
function fetchJsonViaOauthSession(url, options: any = {}): Promise<any> {
  return new Promise((resolve, reject) => {
    const sess = getOauthSession()
...
// ``Authorization: *** (which the gateway gate now accepts) and mints WS
...
async function freshGatewayWsUrl(profile) {
  ...
  if (connection.authMode === 'oauth') {
    ...
    const wsUrl = buildGatewayWsUrlWithTicket(
      connection.baseUrl,
      ticket,
...
async function buildRemoteConnection(
  ...
  remoteIdentity?,
  headers?,
  remoteProfile: null | string = null
) {
...
async function requestJsonForProfile(profile: string | null, path: string, method: string, body?: string) {
  const conn = await ensureBackend(profile)
...
function profileRouteOptions(profile, request?) {
  ...
  return {
    ...
    backendProfile: sshOverride?.remoteProfile || profileRemoteOverride(config, profile)?.remoteProfile,
