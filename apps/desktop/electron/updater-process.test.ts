...
test('resolveStagedApplyScriptHandoff selects the manifest-aware flat script', () => {
  const root = String.raw`C:\Users\hermes\AppData\Local\hermes\hermes-agent`
  const expected = path.join(root, 'scripts', 'desktop-update.ps1')

  assert.deepEqual(
    resolveStagedApplyScriptHandoff(root, {
      isWindows: true,
      fileExists: candidate => candidate === expected
    }),
    {
      command: 'powershell',
      args: ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', expected],
      scriptPath: expected
    }
  )
  assert.equal(resolveStagedApplyScriptHandoff(root, { isWindows: false, fileExists: () => true }), null)
})
