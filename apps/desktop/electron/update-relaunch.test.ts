...
const UNPACKED = POSIX_PATH.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked')
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = POSIX_PATH.join(UNPACKED, 'hermes')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  ...
  assert.equal(resolveUnpackedRelease(POSIX_PATH.join(UNPACKED, 'hermes'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = POSIX_PATH.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'hermes')
