...
export type SessionSwitchTarget = Pick<SessionInfo, 'id' | 'profile'>

const switchTarget = (session?: SessionInfo): null | SessionSwitchTarget =>
  session ? { id: session.id, profile: session.profile } : null
...
export function openOrAdvanceSwitcher(direction: 1 | -1): null | SessionSwitchTarget {
  ...
  let current = sessions.findIndex(
    session => session.id === selectedId && normalizeProfileKey(session.profile) === activeProfile
  )

  if (current === -1) {
    current = sessions.findIndex(session => session.id === selectedId)
  }
  ...
  return switchTarget(sessions[nextIndex])
}

export const highlightedSession = (): null | SessionSwitchTarget =>
  switchTarget($switcherSessions.get()[$switcherIndex.get()])

export const slotSession = (slot: number): null | SessionSwitchTarget =>
  switchTarget(($switcherOpen.get() || pendingBrowse ? $switcherSessions.get() : $sessions.get())[slot - 1])
...
export function commitOnCtrlUp(): null | SessionSwitchTarget {
  clearRevealTimer()
  pendingBrowse = false
