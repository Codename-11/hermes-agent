...
const remoteTarget = {
  ...
  url: 'hermes-remote-file://remote-ssh/srv/report.html'
}

describe('remote HTML previews', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    $connection.set({ mode: 'remote', profile: 'remote-ssh', remoteKind: 'ssh' } as never)
  ...
  it('loads bounded authenticated SSH bytes while retaining the canonical remote URL', async () => {
  ...
  it('falls back to source mode when the SSH transport is not canonical HTML', async () => {
  ...
  it('keeps gateway HTML on the authenticated protocol so relative assets stay remote', () => {
    $connection.set({ mode: 'remote', profile: 'remote-docker', remoteKind: 'url' } as never)
  ...
  it('does not ask Electron main or the data-url bridge to normalize gateway-local paths', async () => {
    ...
    $connection.set({ mode: 'remote', remoteKind: 'url' } as never)
