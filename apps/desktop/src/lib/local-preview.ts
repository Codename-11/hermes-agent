...
async function enrichPreviewTarget(target: PreviewTarget | null): Promise<PreviewTarget | null> {
  ...
  if (target.previewKind === 'html') {
    // URL/cloud gateways can stream the HTML through the authenticated custom
    // protocol, which also keeps relative assets on the owning backend. SSH
    // connections do not have that gateway protocol contract, so use the
    // upstream bounded/sanitized data-URL transport for them.
    if ($connection.get()?.remoteKind !== 'ssh') {
      return target
    }
