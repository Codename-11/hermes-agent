...
describe('resolveVersionStatus', () => {
  ...
  it('names the deploy branch for a counted backend update', () => {
    expect(backend({ behind: 4, branch: 'origin/tgi', version: '0.4.2' }).tooltip).toContain(
      '4 commits behind origin/tgi'
    )
  })
