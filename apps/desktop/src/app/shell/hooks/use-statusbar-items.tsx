...
export function useStatusbarItems({
  ...
  const clientVersionItem = useMemo<StatusbarItem>(() => {
    ...
    const tooltip = [status.tooltip, upstreamDisparityLabel(updateStatus)].filter(Boolean).join(' · ')
  ...
  const backendVersionItem = useMemo<StatusbarItem | null>(() => {
    ...
    const status = resolveVersionStatus({
      ...
      behind: backendUpdateStatus?.deployBehind ?? backendUpdateStatus?.behind ?? 0,
      branch: backendUpdateStatus?.deployBranch ?? backendUpdateStatus?.branch,
      copy,
      remote: true,
    ...
    const tooltip = [
      ...
      upstreamDisparityLabel(backendUpdateStatus)
    ]
      .filter(Boolean)
      .join(' · ')
