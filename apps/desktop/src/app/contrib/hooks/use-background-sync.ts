...
export function resolveActiveTranscriptSession(
  ...
  const ownsSession = (session: { id: string; profile?: string | null }) =>
    sessionMatchesStoredId(session, storedSessionId) && (session.profile?.trim() || 'default') === normalizedProfile

  return $sessions.get().find(ownsSession) ?? $messagingSessions.get().find(ownsSession)
}
