...
import { downloadGatewayMediaFile, isGatewayLocalMediaPath, isRemoteGateway } from '@/lib/media'
    ...
    async (artifact: ArtifactRecord) => {
      try {
        ...
        if (isRemoteGateway() && /^file:/i.test(href)) {
          await downloadGatewayMediaFile(href, artifact.profile ?? null)
...
function ArtifactImageCard({ artifact, failedImage, onImageError, onOpenChat }: ArtifactImageCardProps) {
  ...
  useEffect(() => {
    ...
    setSrc('')
    void artifactImageSrc(artifact.value, artifact.href, artifact.profile ?? null)
      .then(nextSrc => {
        if (active) {
          setSrc(nextSrc)
        }
