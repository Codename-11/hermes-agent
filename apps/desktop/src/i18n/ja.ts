...
export const ja = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      remoteProfilesTitle: 'リモートプロファイル',
      remoteProfilesDesc:
        'このリモートゲートウェイからプロファイルを読み込み、ローカルのプロファイルハンドルとして追加します。各ハンドルはリモートゲートウェイに固定されるため、プロファイルレールでローカル/リモート agent を切り替えられます。',
      loadRemoteProfiles: 'リモートプロファイルを読み込む',
      remoteProfilesFrom: baseUrl => `${baseUrl} のプロファイル`,
      remoteProfilesEmpty: 'このゲートウェイはプロファイルを返しませんでした。',
      remoteProfilesLoadedTitle: 'リモートプロファイルを読み込みました',
      remoteProfilesLoaded: (count, baseUrl) => `${baseUrl} から ${count} 個のプロファイルを読み込みました`,
      remoteProfilesFailed: 'リモートプロファイルの読み込みに失敗しました',
      remoteProfileDefault: 'default',
      remoteProfileNoModel: 'モデル未設定',
      remoteProfileLocal: 'ローカルハンドルあり',
      remoteProfileDefaultTitle: 'Default プロファイルはグローバルです',
      remoteProfileDefaultDesc:
        'リモート default プロファイルは All profiles 接続を使います。個別のレールハンドルが必要な場合は、名前付きリモートプロファイルを作成または選択してください。',
      pinRemoteProfileNew: 'ハンドルを追加',
      pinRemoteProfileExisting: 'リモートに固定',
      remoteProfilePinnedTitle: 'リモートプロファイルを固定しました',
      remoteProfilePinned: name => `${name} はこのリモートゲートウェイを指すようになりました。プロファイルレールから選択して agent を切り替えてください。`,
      remoteProfilePinFailed: name => `${name} を固定できませんでした`,
