...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      remoteProfilesFailed: '遠端 profiles 載入失敗',
      remoteProfileDefault: 'default',
      remoteProfileNoModel: '未設定模型',
      remoteProfileLocal: '本機入口已存在',
      remoteProfileDefaultTitle: 'Default profile 是全域連線',
      remoteProfileDefaultDesc:
        '遠端 default profile 使用「全部 profiles」連線。如果需要單獨的欄位入口，請建立或選擇一個命名的遠端 profile。',
      pinRemoteProfileNew: '新增入口',
      pinRemoteProfileExisting: '固定遠端',
      remoteProfilePinnedTitle: '遠端 profile 已固定',
      remoteProfilePinned: name => `${name} 現在指向此遠端閘道。請從 profile 欄選擇它來切換 agent。`,
      remoteProfilePinFailed: name => `無法固定 ${name}`,
      signedOutTitle: '已登出',
