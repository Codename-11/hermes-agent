...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        Displays the configured status next to the bot name in a thread. Final
        replies may remain top-level while status stays anchored to the trigger.
        If the native API is unavailable, one editable fallback carries progress.
        """
        if self._is_ignored_channel(chat_id):
        ...
        status = ""
        if metadata:
            # ``status_thread_id`` is a dedicated Assistant-status anchor used
            # when final replies intentionally remain top-level. Otherwise use
            # the same synthetic-thread guard as message sending so a top-level
            # event is not accidentally promoted to a Slack Assistant thread.
            thread_ts = metadata.get("status_thread_id") or self._resolve_thread_ts(
        ...
        if not resolved_status or resolved_status == "is thinking...":
            resolved_status = (
            ...
            if not resolved_status:
                ...
                if _elapsed >= 30:
                    ...
                    resolved_status = f"still working… ({_human})"
                else:
                    resolved_status = "is thinking..."
        try:
    ...
    async def stop_typing(self, chat_id: str, metadata=None) -> None:
        ...
        if self._is_ignored_channel(chat_id):
            ...
            for key in [key for key in self._active_status_threads if key[1] == str(chat_id)]:
                self._active_status_threads.pop(key, None)
            for key in [key for key in self._active_status_messages if key[1] == str(chat_id)]:
                self._active_status_messages.pop(key, None)
            return
        if not self._app:
            return
        requested_thread_ts = ""
