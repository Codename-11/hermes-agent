...
def _build_allowed_mentions(config: Optional[PlatformConfig] = None):
    ...
    def _b(name: str, key: str, default: bool) -> bool:
        ...
        return default if not raw else raw in {"true", "1", "yes", "on"}
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _discord_bots_require_inline_mention(self) -> bool:
        ...
        configured = self._gate_raw("bots_require_inline_mention", "DISCORD_BOTS_REQUIRE_INLINE_MENTION")
        if isinstance(configured, bool):
            return configured
        return str(configured or "false").lower().strip() in {"true", "1", "yes", "on"}
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _gate("allow_bots", "DISCORD_ALLOW_BOTS", from_platform_extra=False, lower=True)
    if "bots_require_inline_mention" in discord_cfg:
        ...
        if not _skip_env_bridge:
            _env_default(
                ...
                str(discord_cfg["bots_require_inline_mention"]).lower())
    ...
    if isinstance(allow_mentions_cfg, dict):
        ...
        for yaml_key in ("everyone", "roles", "users", "replied_user"):
            if yaml_key in allow_mentions_cfg and not _skip_env_bridge:
