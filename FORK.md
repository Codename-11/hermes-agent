...
- A deploy update fetches each required ref once per reconciliation phase and propagates explicit fresh-ref state into immediate handoff classification/resolution instead of repeating the same network requests. Transient GitHub 429/5xx and transport failures receive bounded in-transaction backoff; push-race recovery and post-push `origin/tgi` readback remain mandatory freshness gates.
