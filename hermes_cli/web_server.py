...
def _has_valid_session_token(request: Request) -> bool:
    ...
    ``Authorization`` (Caddy ``basic_auth``). Bearer auth accepts the ephemeral
    session token or the stable ``HERMES_GATEWAY_TOKEN`` used by service callers.
    """
    ...
    return bool(gateway_token) and hmac.compare_digest(
        presented, f"Bearer {gateway_token}".encode())
