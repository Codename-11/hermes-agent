...
def _backend_deploy_update_breakdown(limit: int = 20) -> Dict[str, Any]:
    """Separate installable deploy updates from upstream merge disparity.

    ``HEAD..origin/<branch>`` is the artifact a normal Desktop/backend update
    may install. ``origin/<branch>..upstream/main`` is maintenance work for an
    authorized resolver host; it must remain visible without being presented as
    an installable runtime update.
    """
    ...
    if deploy_behind > 0 and limit > 0:
        commits.extend(_commit_log_range("HEAD", remote_ref, limit))

    if deploy_behind > 0:
        message = (
            f"Pending backend update: {deploy_behind} deploy branch "
            f"commit{'s' if deploy_behind != 1 else ''} from {remote_ref}."
        )
    elif upstream_behind > 0:
        message = (
            f"Backend is current with {remote_ref}. upstream/main has "
            f"{upstream_behind} commit{'s' if upstream_behind != 1 else ''} "
    ...
    return {
        ...
        "behind": deploy_behind,
        "update_available": deploy_behind > 0,
        "message": message,
        "commits": commits,
    }
...
async def check_hermes_update(force: bool = False):
    ...
    Returns:
        ...
        commits: for git installs that are behind, a list of the commits
                 ...
                 author, at}. Absent/empty otherwise. The Desktop remote
