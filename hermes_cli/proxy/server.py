...
async def _open_upstream(
    request: "web.Request", rel_path: str, body: bytes, cred: UpstreamCredential,
...
def create_app(adapter: UpstreamAdapter) -> "web.Application":
    ...
    async def handle_models_fallback(request: "web.Request") -> "web.Response":
        return web.json_response({"object": "list", "data": getattr(adapter, "available_models", [])})
    ...
    async def handle_proxy(request: "web.Request") -> "web.StreamResponse":
        ...
        try:
            credential_fn = getattr(adapter, "get_credential_for_request", None)
            cred = await asyncio.to_thread(credential_fn, rel_path, body) if credential_fn else await asyncio.to_thread(adapter.get_credential)
        except Exception as exc:
            logger.warning("proxy: credential resolution failed: %s", exc)
            return _json_error(401, str(exc), code="upstream_auth_failed")
        upstream_path, upstream_body, header_overrides, proxy_context = rel_path, body, {}, {}
        prepare = getattr(adapter, "prepare_proxy_request", None)
        if prepare:
            try:
                upstream_path, upstream_body, header_overrides, proxy_context = prepare(
        ...
        session, upstream_resp = await _open_upstream(
            request, upstream_path, upstream_body, cred, header_overrides)
        ...
        if upstream_resp.status in {401, 429}:
            ...
            try:
                retry_fn = getattr(adapter, "get_retry_credential_for_request", None)
                if retry_fn:
                    retry_cred = await asyncio.to_thread(
                        retry_fn, context=proxy_context, failed_credential=cred,
                        status_code=upstream_resp.status)
                else:
                    retry_cred = await asyncio.to_thread(
                        adapter.get_retry_credential, failed_credential=cred,
                        status_code=upstream_resp.status)
            except Exception as exc:
            ...
            if retry_cred is not None:
                ...
                await session.close()
                session, upstream_resp = await _open_upstream(
                    request, upstream_path, upstream_body, retry_cred, header_overrides)
                if upstream_resp is None:
                    return session
        finalize = getattr(adapter, "finalize_proxy_response", None)
        if finalize:
            transformed = await finalize(request, upstream_resp, session, proxy_context)
