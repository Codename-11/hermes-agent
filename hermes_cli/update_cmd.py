...
def _print_update_brief(
    ...
    *,
    ...
    """Print the fork's categorized brief after a successful git update."""
...
def _desktop_app_present(desktop_dir: Path) -> bool:
    """Return whether this checkout has durable Desktop install intent."""
    return _desktop_install_intent(desktop_dir)
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if is_deploy_branch:
            # The upstream resolver defaults to "main". Keep the shared
            # checkout/pull tail pinned to the live deploy branch after the
            # fork-specific reconciliation has published its tested artifact.
        ...
        if is_deploy_branch:
            ...
            if auto_stash_ref is not None:
                _m()._preserve_deploy_branch_stash(auto_stash_ref)
                # The tested deploy artifact must remain clean; do not let the
                # standard pull cleanup below reapply fork-local source edits.
                auto_stash_ref = None
        else:
            ...
            # the exact count is repaired below through the compare API.
            result = subprocess.run(
