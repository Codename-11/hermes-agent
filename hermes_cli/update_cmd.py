...
def _exit_for_queued_reconciliation() -> None:
    print("\n⏳ Update pending — candidate verification was queued.")
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if (
        not branch_explicit and branch in DEPLOY_BRANCHES
        and _has_upstream_remote(git_cmd, _m().PROJECT_ROOT)
    ):
        remote_ref = f"origin/{branch}"
        print("→ Fetching deploy branch from origin...")
        origin_fetch = _git_run(
            git_cmd, ["fetch"] + depth_args + ["origin", branch], network=True)
        print("→ Fetching from upstream...")
        upstream_fetch = _git_run(
            git_cmd, ["fetch"] + depth_args + ["upstream", "main"], network=True)
        if origin_fetch.returncode != 0 or upstream_fetch.returncode != 0:
            failed = origin_fetch if origin_fetch.returncode != 0 else upstream_fetch
            _print_fetch_failure(failed.stderr)
            sys.exit(1)
        if is_shallow:
            head_sha, deploy_sha = _tip_shas(git_cmd, remote_ref)
            state = "current" if head_sha and head_sha == deploy_sha else "update available"
            print(f"Deploy branch: {state} ({remote_ref}).")
            print("Hermes upstream: fetched; exact disparity unavailable in a shallow checkout.")
            return
        origin_pending = int(_git_run(
            git_cmd, ["rev-list", "--count", f"HEAD..{remote_ref}"], check=True).stdout.strip())
        upstream_pending = int(_git_run(
            git_cmd, ["rev-list", "--count", f"{remote_ref}..upstream/main"], check=True).stdout.strip())
        print(f"Deploy branch: {origin_pending} commit{'s' if origin_pending != 1 else ''} from {remote_ref}.")
        print(
            f"Hermes upstream: {upstream_pending} commit{'s' if upstream_pending != 1 else ''} "
            "awaiting reconciliation into the deploy branch.")
        print("✓ Already up to date." if origin_pending == upstream_pending == 0 else "⚕ Update available.")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        is_deploy_branch = (
            ...
            and current_branch in DEPLOY_BRANCHES and is_fork
            and _has_upstream_remote(git_cmd, _m().PROJECT_ROOT))
        ...
        if is_deploy_branch:
            ...
            deploy_stash_ref = _m()._stash_local_changes_if_needed(git_cmd, _m().PROJECT_ROOT)
            ...
            if not exact_target and _m()._deploy_handoff_exists_for(_m().PROJECT_ROOT, current_branch):
                deploy_commit_count = _m()._resolve_deploy_handoff(
                    git_cmd=git_cmd, repo=_m().PROJECT_ROOT, branch=current_branch,
                    pre_update_head=pre_update_head)
            else:
                deploy_commit_count = _m()._run_deploy_branch_update(
                    git_cmd, _m().PROJECT_ROOT, current_branch, pre_update_head,
                    target_sha=exact_target, legacy_recovery=True)
            if deploy_commit_count is None:
                if deploy_stash_ref is not None:
                    _m()._restore_stashed_changes(
                        git_cmd, _m().PROJECT_ROOT, deploy_stash_ref,
        ...
        _plan = _prepare_checkout_for_update(
            ...
            _windows_gateway_resume=_windows_gateway_resume)
        commit_count = deploy_commit_count if deploy_commit_count is not None else _plan.commit_count
        ...
        pre_pull_sha = pre_update_head if is_deploy_branch else _pull_updates(
