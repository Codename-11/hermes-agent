...
def _confirm_gateway_stable(
    ...
    A single process-table hit only proves the child was created, not that it
    survived startup. Require the gateway to stay visible for the whole
    confirmation window before we vouch for it. Returns the last observed PID
    list, or ``[]`` if the gateway vanished mid-window.
    """
    if confirm_s <= 0:
        return initial_pids
...
def _wait_for_gateway_ready(
    ...
    A first process-table hit is treated as provisional: the gateway must then
    stay visible for ``confirm_s`` more seconds before we report it ready. For
    the active profile, prefer the gateway control socket's authoritative PID;
    the process-table scan stays as the fallback and remains the fleet-wide path
    for ``all_profiles``.
    """
    from gateway.control_socket import identify_gateway
    from hermes_cli.config import get_hermes_home
...
def _report_gateway_start(via: str, *, launched_pid: int | None = None) -> None:
    ...
    if pids:
        ...
        print(
            f"✗ Gateway start via {via}{launcher} FAILED — no stable gateway process "
