...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the live checkout untouched on conflicts. An explicit --branch request
        # opts into the standard branch-target update path below.
        is_deploy_branch = (
            not branch_explicit
        ...
        if is_deploy_branch:
            ...
            if deploy_commit_count > 0:
                ...
                if not syntax_ok:
                    ...
                    if pre_update_head:
                        print()
                        print(f"→ Rolling back to {pre_update_head[:10]}...")
                        rollback_result = subprocess.run(
                            git_cmd + ["reset", "--hard", pre_update_head],
                            cwd=PROJECT_ROOT,
                            capture_output=True,
                            text=True,
                        )
                        if rollback_result.returncode == 0:
                            ...
                            print(f"    cd {PROJECT_ROOT} && git reset --hard {pre_update_head}")
                            if rollback_result.stderr.strip():
                                print(f"    ({rollback_result.stderr.strip().splitlines()[0]})")
                    else:
                        print()
                        print("  Could not capture pre-update SHA — recover manually with:")
        ...
        try:
            ...
            cron_restore = (
                restore_cron_jobs_if_emptied(pre_update_snapshot_id)
                if pre_update_snapshot_id
                else None
            )
            if cron_restore:
        ...
        # Keep a working dashboard alive if Node refresh failed; otherwise
        # restart service-managed dashboards with their original launch arguments.
        if node_failures:
            print()
            print("  ℹ Leaving running dashboard process(es) untouched because the")
            print("    Node.js dependency refresh did not complete.")
