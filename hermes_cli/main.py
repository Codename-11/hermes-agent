...
# ── Axiom fork-only update/deploy helpers ───────────────────────────────
# Extracted from this file into hermes_cli/axiom_update.py to keep the fork's
# footprint out of main.py (upstream's most-refactored module). See
# docs/axiom-fork-contract.md. These names are re-imported here so the
# original call sites in the update flow keep working unchanged.
from hermes_cli.axiom_update import (  # noqa: E402  (fork seam)
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # The web build needs devDependencies (typescript/tsc, vite). When Hermes
    # runs under NODE_ENV=production — common for deployed gateways and any
    # shell that inherits it — npm derives `omit=dev` and skips them, so the
    # later `tsc -b` fails with code 127 and the dashboard silently falls back
    # to a stale dist (the failure is invisible: every `hermes update` "works"
    # but the UI never refreshes). Force `include=dev` for the build-time
    # install so the toolchain is always present regardless of ambient
    # NODE_ENV; `include` wins over a NODE_ENV-derived `omit`. The build step
    # itself still runs with the unmodified env, so `vite build` continues to
    # emit a production bundle. See dashboard-stale-dist regression.
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    exclude: set[int] = set(exclude_pids or set())
    ...
    pids = _find_stale_dashboard_pids(exclude_pids=exclude or None)
    if not pids:
        return
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    # Installer checkouts may be shallow (`git clone --depth 1`). Preserve that
    # boundary when fetching and report presence-only instead of bogus counts.
    is_shallow = (
        subprocess.run(
    ...
    def _fetch(remote: str, ref: Optional[str] = None) -> subprocess.CompletedProcess:
        ...
        cmd = git_cmd + ["fetch"] + depth_args + [remote]
        if ref:
            cmd.append(ref)
        return subprocess.run(
            cmd,
    ...
    if use_deploy_check:
        ...
        if is_shallow:
            ...
            if head_sha and remote_sha and head_sha == remote_sha and (not upstream_sha or upstream_sha == remote_sha):
                print("✓ Already up to date.")
            else:
                print(f"⚕ Update available (behind {remote_ref}{' + upstream/main' if upstream_fetched else ''}).")
                from hermes_cli.config import recommended_update_command
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    if not running_pids and not launcher_pids:
        ...
        # autostart entry (Scheduled Task or Startup-folder login item). If so,
        # cold-start one after the update; otherwise leave gateway-less installs alone.
        try:
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # branch via --branch. Non-deploy channels keep upstream's ordinary
        # branch-switch / fast-forward update behavior below.
        ...
        if is_deploy_branch:
            ...
            if current_branch != branch:
                ...
                # it. Forks fall through to the stash path so intentional edits survive.
                if not is_fork and _clean_managed_worktree(git_cmd, PROJECT_ROOT):
                    auto_stash_ref = None
                else:
                    auto_stash_ref = _stash_local_changes_if_needed(git_cmd, PROJECT_ROOT)
            ...
            if commit_count == 0:
                ...
                if commit_count == 0:
                    ...
                    # A current checkout may still need a managed-runtime
                    # cutover or dependency repair.
                    from hermes_cli.managed_uv import ensure_uv, update_managed_uv
                    ...
                    if not healthy:
                        ...
                        if healthy_after:
                            ...
                            print(
                                "  Close all Hermes windows/gateways and re-run: "
                                "hermes update"
                            )
                    else:
                        print("✓ Already up to date!")
                    ...
                    if runtime_repaired is not None and not _is_windows():
                        print()
                        print(
                            "⚠ Restart required to finish the managed Python "
                            "runtime repair."
                        )
                        print(
                            "  Any running Hermes gateways, Desktop backends, "
                            "or other long-lived processes still use the "
                            "previous runtime."
                        )
                        print(
                            "  Restart each of them before removing the parked "
                            "venv"
                            + (
        ...
        try:
            ...
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        # Collect default, named-profile, and dashboard units.
                        # Keep the upstream per-unit timeout isolation below so one
                        # wedged unit cannot abort the remaining fleet restart.
