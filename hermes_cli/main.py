...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    # Installer checkouts are shallow (`git clone --depth 1`). A plain fetch
    # would unshallow the repo and rev-list would report a bogus huge behind
    # count. Preserve the shallow boundary and report presence-only below.
    is_shallow = (
        subprocess.run(
    ...
    if use_deploy_check:
        ...
        if is_shallow:
            ...
            up_to_date = bool(head_sha and remote_sha and head_sha == remote_sha)
            if upstream_fetched:
                up_to_date = up_to_date and bool(upstream_sha and upstream_sha == remote_sha)
            if up_to_date:
                print("✓ Already up to date.")
            else:
                compare_label = remote_ref + (" + upstream/main" if upstream_fetched else "")
                print(f"⚕ Update available (behind {compare_label}).")
                from hermes_cli.config import recommended_update_command
        ...
        if is_shallow:
            ...
            # and report presence-only (mirrors the banner's _check_via_local_git).
            head_sha = subprocess.run(
                git_cmd + ["rev-parse", "HEAD"],
                cwd=PROJECT_ROOT, capture_output=True, text=True,
            ).stdout.strip()
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    if not running_pids and not launcher_pids:
        ...
        # updates would otherwise never come back until next login. Cold-start
        # one after update if a Windows gateway is installed.
        try:
