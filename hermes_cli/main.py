...
from hermes_cli.axiom_update import (  # noqa: F401
    _clean_managed_worktree, _completed_deploy_handoff_requires_post_update,
    _desktop_shortcut_exists, _detect_windows_gateway_launcher_instances,
    _deploy_handoff_exists_for, _preserve_deploy_branch_stash,
    _resolve_deploy_handoff, _run_deploy_branch_update,
...
def _update_preflight_handled(args) -> bool:
    ...
    if getattr(args, "status", False) or getattr(args, "wait", False):
        ...
        from hermes_cli.main_install_repair import _resolve_reconciliation_branch

        result = show_reconciliation_status(
            _resolve_reconciliation_branch(args), wait=bool(getattr(args, "wait", False)))
        if result:
            sys.exit(result)
        return True
