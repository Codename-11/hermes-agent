...
def _disabled_toolset_block_message(
    function_name: str, disabled_toolsets: Optional[List[str]],
) -> Optional[str]:
    """Return a policy block message when this run disabled the toolset."""
    disabled = {str(name).strip() for name in (disabled_toolsets or []) if str(name).strip()}
    if not disabled:
        return None
    toolset = registry.get_toolset_for_tool(function_name)
    if "*" not in disabled and (not toolset or toolset not in disabled):
        return None
    return (
