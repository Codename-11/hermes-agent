...
class TestApiServerAdapterToolset:
    ...
    @patch("gateway.platforms.api_server.AIOHTTP_AVAILABLE", True)
    def test_create_agent_merges_run_disabled_toolsets(self):
        """Per-run runtime policy restrictions layer on top of config."""
        from gateway.platforms.api_server import APIServerAdapter
        from gateway.config import PlatformConfig
        ...
        with patch("gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, \
            ...
            mock_kwargs.return_value = {"api_key": "test-key", "base_url": None,
                                        ...
                                        "command": None, "args": []}
            mock_model.return_value = "test/model"
            mock_config.return_value = {"agent": {"disabled_toolsets": ["memory"]}}
            mock_agent_cls.return_value = MagicMock()

            adapter._create_agent(disabled_toolsets=["terminal", "file"])

            call_kwargs = mock_agent_cls.call_args
            assert call_kwargs.kwargs.get("disabled_toolsets") == [
                "memory",
                "terminal",
                "file",
            ]
