...
async def test_run_agent_progress_stays_in_originating_topic(monkeypatch, tmp_path):
    ...
    assert all(
        call["metadata"] and call["metadata"].get("thread_id") == "17585"
...
async def test_run_agent_progress_uses_event_message_id_for_slack_dm(monkeypatch, tmp_path):
    ...
    expected_metadata = {
        ...
        "reply_to_message_id": "1234567890.000001",
        "message_id": "1234567890.000001",
    }
    assert adapter.sent[0]["metadata"] == expected_metadata
    assert all(
        call["metadata"]
        and all(call["metadata"].get(key) == value for key, value in expected_metadata.items())
        for call in adapter.typing
    )
