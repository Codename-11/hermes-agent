...
def test_real_read_tool_binaries_confirm_option_ownership(
    ...
    if os.name == "nt":
        pytest.skip("requires native GNU tool option semantics, not MSYS shims")
...
def test_real_binaries_execute_leading_dash_program_payload(
    ...
    if os.name == "nt":
        pytest.skip("requires native GNU executable lookup semantics")
...
def test_max_accepted_separator_free_input_is_fast():
    ...
    limit = 0.50 if os.name == "nt" else 0.15
    assert elapsed < limit, f"max accepted token took {elapsed:.3f}s"
