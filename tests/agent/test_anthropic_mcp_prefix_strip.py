...
class TestAnthropicOAuthClassifierAlias:
    ...
    def test_oauth_aliases_session_search_in_system_prompt_prose(self):
        ...
        # TGI's OAuth compatibility path leaves system[] as the Claude Code
        # identity only and relocates sanitized prompt prose to the first user
        # message, where it remains cache-marked.
        first_user = next(m for m in kwargs["messages"] if m["role"] == "user")
    ...
    def test_oauth_does_not_alias_longer_identifier_containing_token(self):
        ...
        first_user = next(m for m in kwargs["messages"] if m["role"] == "user")
        preamble = first_user["content"][0]["text"]
        assert "tools/session_search_tool.py" in preamble
