...
@pytest.mark.parametrize(
    ...
    [
        (0, ("clean_exit", 0)),
        (kbd._kb.KANBAN_RATE_LIMIT_EXIT_CODE << 8,
         ("rate_limited", kbd._kb.KANBAN_RATE_LIMIT_EXIT_CODE)),
        (7 << 8, ("nonzero_exit", 7)),
        (9, ("signaled", 9)),
    ],
)
def test_decode_worker_wait_status_without_posix_helpers(monkeypatch, raw_status, expected):
    for name in ("WIFEXITED", "WEXITSTATUS", "WIFSIGNALED", "WTERMSIG"):
        monkeypatch.delattr(kbd.os, name, raising=False)
    assert kbd._decode_worker_wait_status(raw_status) == expected


def test_reap_worker_zombies_exercises_mocked_waitpid_on_windows(monkeypatch):
    status = kbd._kb.KANBAN_RATE_LIMIT_EXIT_CODE << 8
    waitpid = unittest.mock.Mock(side_effect=[(12345, status), (0, 0)])
    record_exit = unittest.mock.Mock()
    monkeypatch.setattr(kbd.os, "name", "nt")
    monkeypatch.setattr(kbd.os, "waitpid", waitpid)
    monkeypatch.setattr(kbd, "_record_worker_exit", record_exit)
    assert kbd.reap_worker_zombies() == [12345]
...
def test_reap_worker_zombies_stays_noop_for_real_windows_waitpid(monkeypatch):
    ...
    monkeypatch.setattr(kbd.os, "name", "nt")
    monkeypatch.setattr(kbd.os, "waitpid", waitpid)
    assert kbd.reap_worker_zombies() == []
