...
def test_checkpoint_ignores_whitespace_in_non_conflict_upstream_paths(
    monkeypatch, tmp_path
):
    from hermes_cli import axiom_update
    ...
    subprocess.run(["git", "config", "user.email", "test@example.com"], cwd=repo, check=True)
    subprocess.run(["git", "config", "user.name", "Test"], cwd=repo, check=True)
    (repo / "resolved.txt").write_text("before\n", encoding="utf-8")
    (repo / "upstream.txt").write_text("before\n", encoding="utf-8")
    subprocess.run(["git", "add", "."], cwd=repo, check=True)
    subprocess.run(["git", "commit", "-m", "base"], cwd=repo, check=True)

    (repo / "resolved.txt").write_text("after\n", encoding="utf-8")
    (repo / "upstream.txt").write_text("inherited upstream whitespace   \n", encoding="utf-8")
    subprocess.run(["git", "add", "."], cwd=repo, check=True)
    marker = tmp_path / ".update_handoff.json"
    marker.write_text(
        json.dumps({"phase": "resolve_pending", "conflict_files": ["resolved.txt"]}),
        encoding="utf-8",
    )
    monkeypatch.setattr(axiom_update, "_deploy_handoff_marker_path", lambda: marker)
    ...
    assert error == ""
    assert len(resolved_head) == 40
    assert json.loads(marker.read_text(encoding="utf-8"))["phase"] == "validation_pending"
